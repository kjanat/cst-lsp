"""
E2E tests for error handling and invalid request scenarios using direct client.
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_invalid_range_handling_direct(simple_project):
    """
    Test server handles invalid position ranges gracefully using direct client.
...
"""
    ...
    try:
        ...
        result = await client.text_document_code_action(params_reversed)
        ...
        # Should not crash
        result = await client.text_document_code_action(params_out_of_bounds)
        ...
        result = await client.text_document_code_action(params_valid)

        # Server should respond (may or may not have actions, but should respond)
        assert result is not None or True, (
            "Server failed to respond after error scenarios"
        )
